...
import emul_java.awt.*;
import emul_java.awt.image.PixelGrabber;
...
//        rd.drawImage(fleximg, 0, 0, null);
    ...
    //            runner.start();
        ...
        if (!loadedt[i - 1]) {
            stracks[i - 1] = app.loadMod("music/stage" + i + ".radq");
            if (stracks[i - 1].isLoaded()) {
                loadedt[i - 1] = true;
            }
        }
        if (i == 1) {
...
//            smokey = loadWebImage("smokey.gif");
    ...
    private Image loadWebImage(String s) {
        return app.loadImage("data/images/" + s);
    }
...
//        return createImage(new MemoryImageSource(j, i, ai, 0, j));
    ...
    public xtGraphics(Graphics2D graphics2d, GameSparker applet) {
        ...
        app = applet;
        rd = graphics2d;
        hello = app.loadImage("data/hello.gif");
        sign = app.loadImage("data/sign.gif");
        loadbar = app.loadImage("data/loadbar.gif");
        int i = 0;
        do {
            loadedt[i] = false;
        } while (++i < 17);
...
//        runner.start();
        ...
        cars = app.loadMod("music/cars.radq");
        dnload += 27;

        stages = app.loadMod("music/stages.radq");
        dnload += 91;
    ...
    public void loadsounds() {
        ...
        for (i = 0; i < 5; ++i) {
            for (int v = 0; v < 5; ++v) {
                sm.add("engs" + v + i, app.loadSound(v + "" + i + ".wav"));
            }
        }

        for (i = 0; i < 6; ++i) {
            sm.add("air" + i, app.loadSound("air" + i + ".wav"));
        }

        for (i = 0; i < 3; ++i) {
            sm.add("crash" + i, app.loadSound("crash" + (i + 1) + ".wav"));
        }

        for (i = 0; i < 3; ++i) {
            sm.add("lowcrash" + i, app.loadSound("lowcrash" + (i + 1) + ".wav"));
        }

        for (i = 0; i < 3; ++i) {
            sm.add("skid" + i, app.loadSound("skid" + (i + 1) + ".wav"));
        }

        for (i = 0; i < 3; ++i) {
            sm.add("dustskid" + i, app.loadSound("dustskid" + (i + 1) + ".wav"));
        }

        sm.add("powerup", app.loadSound("powerup.wav"));
        sm.add("tires", app.loadSound("tires.wav"));
        sm.add("checkpoint", app.loadSound("checkpoint.wav"));
        sm.add("carfixed", app.loadSound("carfixed.wav"));
        sm.add("three", app.loadSound("three.wav"));
        sm.add("two", app.loadSound("two.wav"));
        sm.add("one", app.loadSound("one.wav"));
        sm.add("go", app.loadSound("go.wav"));
        sm.add("wasted", app.loadSound("wasted.wav"));
        sm.add("firewasted", app.loadSound("firewasted.wav"));
