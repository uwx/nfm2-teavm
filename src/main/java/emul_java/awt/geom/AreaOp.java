...
package emul_java.awt.geom;

import emul_java.lang.InternalError;
...
public abstract class AreaOp {
			...
			for (cur = left; cur < right; cur++) {
				...
				for (next = cur; next > left; next--) {
					...
					if (yrange[1] <= yrange[0]) {
						throw new emul_java.lang.InternalError("backstepping to " + yrange[1] + " from " + yrange[0]);
					}
					if (ordering >= 0) {
						if (ordering == 0) {
	...
	public static void finalizeSubCurves(Vector subcurves, Vector chains) {
		...
		if ((numchains & 1) != 0) {
			throw new emul_java.lang.InternalError("Odd number of chains!");
		}
	...
	public static void resolveLinks(Vector subcurves, Vector chains, Vector links) {
		...
		if (numlinks == 0) {
			...
			if ((numlinks & 1) != 0) {
				throw new emul_java.lang.InternalError("Odd number of new curves!");
			}
