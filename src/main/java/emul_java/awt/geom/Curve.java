...
public abstract class Curve {
	...
	public int compareTo(Curve that, double yrange[]) {
		...
		if (y1 <= yrange[0]) {
			...
			throw new emul_java.lang.InternalError("backstepping from " + yrange[0] + " to " + y1);
		}
		yrange[1] = y1;
	...
	public boolean findIntersect(Curve that, double yrange[], double ymin, int slevel, int tlevel, double s0,
		...
		if (s1 - s0 > TMIN) {
			...
			if (s == s0 || s == s1) {
				...
				throw new emul_java.lang.InternalError("no s progress!");
			}
			if (t1 - t0 > TMIN) {
				double t = (t0 + t1) / 2;
				...
				if (t == t0 || t == t1) {
					...
					throw new emul_java.lang.InternalError("no t progress!");
				}
				if (ys >= yt0 && yt >= ys0) {
